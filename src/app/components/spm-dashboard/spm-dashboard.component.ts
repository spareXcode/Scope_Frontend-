...
@Component({
  ...
  standalone: true,
  imports: [SideMenueComponent,CalendarModule,FormsModule,OrderRequestStatusCardComponent,TopNavBarComponent,MainNavbarComponent,ReactiveFormsModule,CommonModule,
...
export class SpmDashboardComponent {
  ...
  onSelectionChange(){
    ...
    this.fetchSpmDashBoardDataSingleLocation(this.spmDashBoardInputData.value)
  ...
  locationData: any = []
  spmDashBoardDataAllLocatoin: any = []
  spmDashboardDataSingleLocation: any = []
  ...
  fetchStockUploadData(locationId:any) {
    ...
    fetchSpmDashBoardDataAllLocation(data:any){
    this.isLoading = true
    this.Dashboardservice.getSpmDashBoardAllLocationData({dealer_id:this.dealer_id,timeBand:this.spmDashboardData.Timeband}).subscribe((res:any)=>{
      this.sumOfNotInMaster = res.data?.partNotInMasterData?.partNotInMasterCount || 0
      this.sumofRejectedRequest = res.data?.noOfRequestRejectedData?.rejectedCount || 0 
      this.sumofApprovedRequest = res.data?.noOfRequestApprovedData?.approvedCount || 0
      this.sumOfPendingRequest = res.data?.noOfRequestPendingData?.pendingCount  || 0
      this.isLoading = false;
    })
  }
  
  fetchSpmDashBoardDataSingleLocation(data:any){
    this.isLoading = true
    let locationIds=[this.spmDashboardData.locationID]
    this.Dashboardservice.getSpmDashboardSingleLocationData({dealer_id:this.dealer_id,location_id:locationIds,timeBand:this.spmDashboardData.Timeband}).subscribe((res:any)=>{
      this.sumOfNotInMaster = res.data?.partNotInMasterData?.partNotInMasterCount || 0
      this.sumofRejectedRequest = res.data?.noOfRequestRejectedData?.rejectedCount || 0 
      this.sumofApprovedRequest = res.data?.noOfRequestApprovedData?.approvedCount || 0
      this.sumOfPendingRequest = res.data?.noOfRequestPendingData?.pendingCount  || 0
  ...
  ngOnInit(): void {
    ...
    this.fetchSpmDashBoardDataAllLocation(this.dealer_id)
